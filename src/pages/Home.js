...
import { FiArrowRight, FiGift, FiTruck, FiBell, FiCreditCard } from 'react-icons/fi';
    ...
    <div className="home">
      ...
      <section className="trending">
        ...
        <div className="section-cta">
          <button className="btn-secondary" onClick={() => navigate('/explore')}>
            View All Gifts <FiArrowRight style={{marginLeft:'8px'}} />
          </button>
        </div>
      </section>
      ...
      <section className="occasions">
        ...
        <div className="occasion-grid">
          {[
            { label: 'Birthday', filter: 'Birthday', emoji: '🎂' },
            { label: 'Anniversary', filter: 'Anniversary', emoji: '💑' },
            { label: "Valentine's", filter: "Valentine's", emoji: '💝' },
            { label: 'Wedding', filter: 'Wedding', emoji: '💍' },
            { label: 'Baby Shower', filter: 'Baby Shower', emoji: '👶' },
            { label: 'Festival', filter: 'Festival', emoji: '🪔' },
            { label: 'Corporate', filter: 'Corporate', emoji: '💼' },
            { label: 'General', filter: 'General', emoji: '🎁' },
          ].map((item, index) => (
            <div
              className="occasion-card"
              key={index}
              onClick={() => navigate(`/explore?occasion=${item.filter}`)}
              style={{cursor: 'pointer'}}
            >
              <span className="occasion-emoji-text">{item.emoji}</span>
              <p>{item.label}</p>
            </div>
          ))}
        </div>
